...
function employeePortraitMarkup(employee = {}, label = 'AI', extraClass = '') {
  const id = employee.id || employee.employee_id || '';
  const color = employee.color || '#7ee8ff';
  const initials = String(label || employee.name || 'AI').split(/\s+/).map((part) => part[0]).join('').slice(0, 2).toUpperCase() || 'AI';
  const src = employee.avatar_url || (id ? `/static/assets/employee-avatars/${encodeURIComponent(id)}.webp` : '');
  return `<span class="employee-profile ${extraClass}" style="--employee-color:${safe(color)}">${src ? `<img src="${safe(src)}" alt="" loading="lazy" onerror="this.hidden=true;this.nextElementSibling.hidden=false">` : ''}<span class="profile-initials"${src ? ' hidden' : ''}>${safe(initials)}</span></span>`;
}
